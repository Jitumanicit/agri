...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($fname_err) && empty($lname_err) && empty($email_err) && empty($mobile_err) && empty($sector_err) && empty($city_err) && empty($state_err) && empty($pin_err) && empty($country_err)){
        ...
        $sql = "INSERT INTO tbl_users (fname, mname, lname, designation, organisation, email, mobile, sector, others, organisation_turnover, organisation_profile, address1, address2, city, state, pin, country, register_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssssssssss", $param_fname, $param_mname, $param_lname, $param_designation, $param_organisation, $param_email, $param_mobile, $param_sector, $param_others, $param_organisation_turnover, $param_organisation_profile, $param_organisation_address1, $param_organisation_address2, $param_city, $param_state, $param_pin, $param_country,  $param_register_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                echo 'setTimeout(function () { swal("Thank You!","Your name has been Pre-Registered for the Event.Confirmation of participation subject to further approval. Your Unique ID No is '.$param_register_id.'","success");';
                echo '}, 300);</script>';
            } else{
                  header("refresh: 5");
                  echo '<script type="text/javascript">';
...
<div class="wrapper" style="max-width: 700px; padding: 20px; margin: 0 auto; box-shadow: 0 0 10px rgba(0,0,0,0.6);">
    <p><i class="fa fa-hand-o-right"></i>&nbsp;All conference delegates in the AIAHS 2020 need to fill up and submit registration proforma online at www.agrihortishow.com</p>
    <p><i class="fa fa-hand-o-right"></i>&nbsp;After submission of proforma, a notification confirming submission with unique ID will be sent.</p>
    <p><i class="fa fa-hand-o-right"></i>&nbsp;After submission all delegates will be pre-registered for the conference subject to further approval.</p>
    <p><i class="fa fa-hand-o-right"></i>&nbsp;Upon approval, participants will receive notification on mail/sms.</p>
    <p><i class="fa fa-hand-o-right"></i>&nbsp;Once approval notification received. Now Register for B2B/B2G portal<a href="register-for-b2b.php" style="color: red;"> Register Now</a></p>
</div>
<div class="wrapper" style=" max-width: 700px; padding: 20px; margin: 0 auto; margin-top: 20px; margin-bottom: 50px; box-shadow: 0 0 10px rgba(0,0,0,0.6); ">
        <h2 style="text-align: center;">Register as a Delegate</h2><hr>
        <p style="margin-bottom: 20px; text-align: center;">Please fill this form to submit your interest for participation</p>
